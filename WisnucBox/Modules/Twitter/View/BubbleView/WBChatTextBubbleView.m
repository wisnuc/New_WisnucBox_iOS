...
//　textLaebl 最大宽度
CGFloat const TEXTLABEL_MAX_WIDTH_X = 200.0f;
CGFloat const LABEL_FONT_SIZE_X = 15.0f;

static CGSize kTextBoundingSize;

@interface WBChatTextBubbleView () {
...
@implementation WBChatTextBubbleView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        _textLabel.font = [UIFont systemFontOfSize:LABEL_FONT_SIZE_X];
        _textLabel.backgroundColor = [UIColor clearColor];
        _textLabel.userInteractionEnabled = NO;
        _textLabel.multipleTouchEnabled = NO;
        _textLabel.textColor = UICOLOR_RGB(0x1d1d1d);
...
- (void)setMessageModel:(WBTweetModel *)messageModel {
    ...
    NSAttributedString *text = [WBChatTextBubbleView processModel:self.messageModel];
    YYTextContainer *container = [YYTextContainer containerWithSize:CGSizeMake(TEXTLABEL_MAX_WIDTH_X, MAXFLOAT)];
...
+ (NSAttributedString *)processModel:(WBTweetModel *)model {
    ...
    NSDictionary *attri = [NSDictionary dictionaryWithObjects:@[[UIFont systemFontOfSize:15],model.isSender ? [UIColor whiteColor] : UICOLOR_RGB(0x47474a),paragraphStyle] forKeys:@[NSFontAttributeName,NSForegroundColorAttributeName,NSParagraphStyleAttributeName]];
    [mAttributedString appendAttributedString:[[NSAttributedString alloc] initWithString:model.comment attributes:attri]];
    ...
    if (!regularExpression) {
        ...
        while (index > 0) {
            ...
            if (imageName) {
                //获取图片
                YYImage * image = [WBChatTextBubbleView getImageWithRangeString:imageName];//这是个自定义的方法
                if (image != nil) {
                    YYAnimatedImageView *imageView = [[YYAnimatedImageView alloc] initWithImage:image];
                    imageView.width = 50;
                    imageView.height = 50;
                    
                    NSMutableAttributedString *attachText = [NSMutableAttributedString attachmentStringWithContent:imageView contentMode:UIViewContentModeCenter attachmentSize:imageView.size alignToFont:[UIFont systemFontOfSize:15] alignment:YYTextVerticalAlignmentCenter];
                    //开始替换
...
+ (UIFont *)textLabelFont {
    return [UIFont systemFontOfSize:LABEL_FONT_SIZE_X];
}
