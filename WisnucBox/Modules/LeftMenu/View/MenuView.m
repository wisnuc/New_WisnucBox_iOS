...
-(UIWindow *)menuWindow{
    if (!_menuWindow) {
        ...
        _menuWindow.windowLevel = UIWindowLevelStatusBar+1;
...
-(instancetype)initWithDependencyView:(UIView *)dependencyView MenuView:(UIView *)leftmenuView isShowCoverView:(BOOL)isCover{
    ...
    if(self = [super init]){
        ...
        [leftmenuView addGestureRecognizer:panGestureRecognizer];
...
//        [self.leftMenuView addGestureRecognizer:panGestureRecognizer];
        _coverView = Cover;
    }
    return _coverView;
}
...
-(void)handleftPan:(UIPanGestureRecognizer*)recognizer{
    ...
    if(UIGestureRecognizerStateBegan == recognizer.state ||
       UIGestureRecognizerStateChanged == recognizer.state){
        //Printing description of self->_coverView:  <UIView: 0x150654a00; frame = (218 0; 157 667); alpha = 0.508667; gestureRecognizers = <NSArray: 0x150656500>; layer = <CALayer: 0x1500945e0>>
        //
...
//            NSLog(@"%f,%f,%f,%f",self.leftMenuView.frame.size.width-Place,0.01,kSCREEN_WIDTH-self.leftMenuView.frame.size.width+Place,self.menuViewframe.size.height);
            ...
            //93.000000,0.010000,282.000000,667.000000
            CGFloat x           = 0;
            CGFloat y           = self.menuViewframe.origin.y;
            CGFloat w           = self.menuViewframe.size.width;
            CGFloat h           = self.menuViewframe.size.height;
