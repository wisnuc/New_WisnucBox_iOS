...
+ (MWPhoto *)photoGetQualityImageWithHash:(NSString*)photoHash BoxUUID:(NSString *)boxuuid{
    return [[MWPhoto alloc] initWithHash:photoHash BoxUUID:boxuuid];
}

+ (MWPhoto *)photoHighImageWithHash:(NSString*)photoHash{
    return [[MWPhoto alloc] initWithHash:photoHash];
}

//getTweeethighQualityImageWithHash
...
- (id)initWithHash:(NSString*)photoHash{
    if ((self = [super init])) {
...
- (id)initWithHash:(NSString*)photoHash BoxUUID:(NSString *)boxuuid{
    if ((self = [super init])) {
...
- (void)performLoadUnderlyingImageAndNotify {
    ...
    if (_image) {
        ...
        [self _performLoadUnderlyingImageAndNotifyWithPhotoHah:_photoHash BoxUUID:_boxuuid];
...
- (void)_performLoadUnderlyingImageAndNotifyWithPhotoHah:(NSString *)photoHash BoxUUID:(NSString *)boxuuid{
     @try {
    [WB_NetService getTweeethighQualityImageWithHash:photoHash BoxUUID:boxuuid complete:^(NSError *error, UIImage *image) {
        if (error) {
            MWLog(@"SDWebImage failed to download image: %@", error);
...
- (void)_performLoadUnderlyingImageAndNotifyWithPhotoHah:(NSString *)photoHash{
    @try {
        [WB_NetService getHighWebImageWithHash:photoHash completeBlock:^(NSError *error, UIImage *image) {
            if (error) {
                MWLog(@"SDWebImage failed to download image: %@", error);
